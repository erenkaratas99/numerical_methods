...
for cnt in range(0,30):
    ...
    x1_list.append((2 + x2_list[cnt] - x3_list[cnt] + x4_list[cnt])/-4)
    x2_list.append((-x1_list[cnt+1] + x3_list[cnt] + x4_list[cnt] - 1)/4)
    x3_list.append((x1_list[cnt+1] + x2_list[cnt+1] - x4_list[cnt])/5)
    x4_list.append((-1 + x1_list[cnt+1] + x3_list[cnt+1] - x2_list[cnt+1])/-3)
...
writer = pd.ExcelWriter('Gauss_Seidel.xlsx', engine='xlsxwriter')
main_df.to_excel(writer, sheet_name='gauss_seidel', index=False)
writer.save()
